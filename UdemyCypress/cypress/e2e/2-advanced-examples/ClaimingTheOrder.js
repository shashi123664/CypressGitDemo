describe('claming the order and verify in your claim queue script', function(){


  it('claiming the order and verify in your claim queue test case',function(){
    ...
    cy.get('[class="col-md-12 col-lg-8 pr-lg-1"]').find('[class="mb-2"]').eq(0).then(function(ActualOrderNumber){
      ...
      cy.get('#order-queue-dropdownMenuButton').click()
        cy.get('.border-0 > :nth-child(1) > .d-flex > :nth-child(1) > .mb-1').click()
        cy.wait(3000)
        cy.get('[class="col-md-12 col-lg-8 pr-lg-1"]').find('[class="mb-2"]').each(($el , index , $list) =>{
          if($el.text().includes(orderNumber))
          {
            orderNumberInClaimedQueue  = orderNumber
            cy.log(orderNumber)
            cy.log(orderNumberInClaimedQueue )
          }
        })
      ...
      expect(orderNumberInClaimedQueue).to.equals(ordernumberInOrderApproval)
    })
    })
